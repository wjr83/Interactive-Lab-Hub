...
def initialize_scales(mux):
    ...
    for port, label in zip(ports, ports_class.keys()):
    ...
    # Create a list of [port, address] for each key in ports_class
    result_list = [[ports_class[key], scale[key]] for key, scale in [(key, value) for value in scales for key in value.keys()] if key in ports_class]

    # Update ports_class with the new lists
    for key, value in result_list:
        ports_class[key] = [value]

    # Print the updated ports_class
    print(ports_class)
